  ...
  try {
    ...
    dataKeys.forEach((dataKey) => {
      ...
      if (!(dataKey in secret.data)) {
        console.log(
          `Key '${dataKey}' not found in '${taskParameters.secretPath}'`
        );
      } else {
        ...
        if (!dataValue) {
          ...
          let value: string =
            type === 'object' ? JSON.stringify(dataValue) : dataValue;
          tl.setVariable(name, value, false);
          console.log(
            `Stored value of type '${type}' for key '${dataKey}' in the variable '${name}'`
          );
        }
      }
    });
